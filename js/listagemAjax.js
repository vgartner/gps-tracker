...
function saveAddressNovo(row, add, lat, lng, attempts){
	$(row).removeClass('nAdrr');
	$(row).addClass('wAdrr');
	$(row).attr('data-pos',lat+''+lng);
	var tds = $(row).find('td')
...
function getAddressNovo(attempts){
	//console.log('INICIO CON attempts : ' + attempts)
	var row = $('tr.nAdrr:first')
	if (row.size() == 1) {
		var tds = $(row).find('td')
		var lat = $.trim($(tds[6]).text())
		lat = Math.round(lat * 100000) / 100000;
		var lng = $.trim($(tds[7]).text())
		lng = Math.round(lng * 100000) / 100000;
		...
		if ( cacheRow.size() == 1 ) {
			var tdsc = $(cacheRow).find('td')
			var add = $(tdsc[2]).text()
			saveAddressNovo(row, add, lat, lng, attempts);
		}else{
			if (attempts < 3) {
				var geocoder = new google.maps.Geocoder();
				geocoder.geocode( { 'address': lat+","+lng},
				function(results, status){
					switch(status){
						case 'OK':
							var add = results[0].formatted_address;
							saveAddressNovo(row, add, lat, lng, 0);
							break;
						case 'OVER_QUERY_LIMIT':
						case 'REQUEST_DENIED':
							setTimeout( function f03(){
								getAddressNovo(attempts + 1)
							}, 1000 )
							break;
						default:
							cancelRowNovo(row);
...
function bindGridNovo(str)
{
	if (str == '') {
		...
		"</tr>"+
		"</thead>"+
		"<tbody><tr class=''><td colspan='9' align='center'> Selecione um veículo no menu. </td></tr></tbody></table>";
	} else {
		strLocal = str;
		...
		var url="listagem_novo.php";
		url=url+"?imei="+str;
		//url=url+"&sid="+Math.random();
		xmlhttp.onreadystatechange = stateChangedNovo;
		xmlhttp.open("GET", url, true);
		xmlhttp.send(null);
